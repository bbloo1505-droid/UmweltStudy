...
export function DebugDataBanner() {
  ...
  return (
    <div className="border-b bg-amber-50 px-4 py-2 text-xs text-amber-900 print:hidden">
      <span className="font-semibold">Debug:</span> glossary={glossaryCards.length} • library={libraryEntries.length} •
      interview={interviewQuestions.length} • scenarios={scenarios.length} • mcq={mcqQuestions.length}
    </div>
  )
}
